...
(function ($) {
            ...
            return this.each(function () {
                ...
                if (flashInstalled) {
                    ...
                    swfuploadify.queueData = {
                        ...
                        errorMsg: "文件格式不正确！支持格式为:jpg/gif/jpeg/png/bmp"
                    };
    ...
    var handlers = {
        ...
        onDialogOpen: function () {
            ...
            this.queueData.errorMsg = "文件格式不正确！支持格式为:jpg/gif/jpeg/png/bmp";
        ...
        onSelect: function (file) {
            ...
            for (var n in this.queueData.files) {
                ...
                if (queuedFile.uploaded != true && queuedFile.name == file.name) {
                    var replaceQueueItem = confirm('服务器上已经存在名为 "' + file.name + '"的文件.\n 是否要替换现有文件?');
                    if (!replaceQueueItem) {
            ...
            if (settings.checkExisting) {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == 1) {
                            var overwrite = confirm('服务器上已存在名为' + file.name + '" 的文件.\n 是否要替换现有的文件?');
                            if (!overwrite) {
                                this.cancelUpload(file.id);
                                $('#' + file.id).remove();
